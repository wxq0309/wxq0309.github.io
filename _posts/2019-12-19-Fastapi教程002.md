...
### Path参数接收 `path` 文件路径
  ...
  举个例子： 我们现在有一个路径参数 `/files/{file_path}`,但是 `file_paht`表示需要传一个文件，文件路径是本地的`home/johndoe/myfile.txt`,所以最后的`Path`路径看起来是 `/files/home/johndoe/myfile.txt`。这种方式在Fastapi中是不被支持的，因此需要稍微改动一下，将`/files/{file_path}`改为`/files/{file_path:path}`, 代码如下：
